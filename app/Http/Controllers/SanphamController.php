...
class SanphamController extends Controller
{
    public function index()
    {
        ...
        return view('admin.indexsanpham', compact('sp'))->with('i',(request()->input('page',1)-1)*5);
    }
    ...
    {    
        if ($request->isMethod('post'))
        {
                ...
                if (!$Sanpham)
                {
                    ...
                    //     \Log::error(sprintf('%s: %s Failed: %s', __CLASS__, __FUNCTION__, $exception->getMessage()), $exception->getTrace());
                    // }
                   if($value)
                   {
